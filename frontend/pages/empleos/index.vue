        ...
        <div class="row">
          <div class="col-lg-12">
            <div class="breadcaump-inner text-center">
              <h1 class="heading heading-h1">{{ $t("jobs") }}</h1>
            </div>
          </div>
        </div>
      </div>
          ...
          <div class="container">
            <div class="row mtn--50">
              <div
                v-for="job in jobs"
                :key="job.name"
                class="col-md-12 move-up wow mt--50"
              >
                <JobPost
                  :job="job"
...
export default {
  components: {
    JobPost: () => import("@/components/JobPost"),
  },
  data() {
    return {
      page: 1,
  ...
  watch: {
    '$route'() {
      ...
      pageTrackGAV4(this.$gtag, window.location.pathname, `${this.$t("jobs")} | ${pageTitle}`, window.location.href);
      screenViewGAV4(this.$gtag, `${this.$t("jobs")} | ${pageTitle}`, siteUid);
