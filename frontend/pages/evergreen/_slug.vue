        ...
        >
          <div class="portfolio-left bk-portfolio-details">
            <div class="portfolio-main-info">
              ...
              <div class="portfolio-details-list mt-4">
              ...
              <h3
                v-if="news.length"
                class="mt--50 border-top pt-4 border-muted"
              >
                 More Travel Selling Trips
              </h3>
              <div class="row mt--0">
                <div
                  v-for="item in news"
                  :key="item.id"
                  class="col-xl-3 col-md-4 col-sm-6 move-up wow"
                >
                  <BlogPostThree
...
export default {
  ...
  async asyncData({ $strapi, params, query, $config: { siteUid } }) {
    const matchingArticles = await $strapi.find("evergreens", {
      slug: params.slug,
    });
    return {
      ...
      news: await $strapi.find(
        ...
        qs.stringify({
          ...
          "sites.uid": siteUid,
          status: "published",
          id_lt: matchingArticles[0].id, 
          publishedDate_lte: matchingArticles[0].publishedDate, 
          id_ne:matchingArticles[0].id,
          _where: {
            _or: [
