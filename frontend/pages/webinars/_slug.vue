    ...
    <div class="brook-portfolio-details bg_color--1">
      <div class="container">
        <BreadcrumbSlug
          path="/webinars"
          slag
          :crumb="typeWebinars ? $t('upcomingwebinar') : $t('lastwebinar')"
          :title="article.title"
        />
        <div class="row pb--40">
          <div
          ...
          >
            <div class="portfolio-left bk-portfolio-details">
              <div class="portfolio-main-info">
                ...
                <div v-show="article.link">
                  <a target="_black" :href="article.link" class="">
                    <button
                      type="button"
...
export default {
  ...
  async asyncData({ $strapi, params, query, $config: { siteUid } }) {
    const matchingArticle = await $strapi.find("webinars", {
      slug: params.slug,
      "sites.uid": siteUid,
    });
    return {
  ...
  computed: {
    typeWebinars() {
      if (new Date(this.article.publishedDate) > new Date()) return true;
      else return false;
    },
  },
  created() {
